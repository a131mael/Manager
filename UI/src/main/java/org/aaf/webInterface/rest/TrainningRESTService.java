...
import org.aaf.webInterface.service.TrainningService;
...
@Path("/trainnings")
@RequestScoped
public class TrainningRESTService {
	...
	@Path("/{idTeam:[0-9][0-9]*}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getTrainning(@PathParam("idTeam") long idTeam) {
		Response.ResponseBuilder builder = null;
		
		builder = Response.ok();
		
		Training training = trainningService.getTrainning(idTeam);
		if (training == null) {
			builder = Response.status(Response.Status.BAD_REQUEST).entity("erro");
			throw new WebApplicationException(Response.Status.NOT_FOUND);
		} else {
			builder = Response.ok();
			builder.entity(JsonWriter.objectToJson(Convertes.getTrainning(training)));
		}
